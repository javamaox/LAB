...
public class CnToSpell {
	...
	public static String getHumpSpell(String cnStr){
		if (null == cnStr || "".equals(cnStr.trim())) {
			return cnStr;
		}
		char[] chars = cnStr.toCharArray();
		...
		for (int i = 0, Len = chars.length; i < Len; i++) {
			...
			if (ascii == 0) { // 取ascii时出错
				...
				if (spell == null) {
					...
					// System.out.println("spell:"+spell);
					retuBuf.append((spell.charAt(0)+"").toUpperCase()+spell.substring(1));
	...
	public static String getFirstSpell(String cnStr) {
		cnStr = cnStr.trim();
		String res="";
		int hzpysum = cnStr.length();
		for (int i = 0; i < hzpysum; i++) {
			byte[] bytes = (cnStr.charAt(i) + "").getBytes();
			int code = (bytes[0] << 8 & 0xff00) + (bytes[1] & 0xff);
