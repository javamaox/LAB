...
function Datepicker() {
	...
	this.regional[''] = { // Default regional settings
		...
		clearStatus: '清空已选日期', // Status text for clear link
		closeText: '关闭', // Display text for close link
		closeStatus: '不改变当前选择', // Status text for close link
		prevText: '&#x3c;上月', // Display text for previous month link
		prevStatus: '显示上月', // Status text for previous month link
		nextText: '下月&#x3e;', // Display text for next month link
		nextStatus: '显示下月', // Status text for next month link
		currentText: '本月', // Display text for current month link
		currentStatus: '显示本月', // Status text for current month link
		monthNames: ['一月','二月','三月','四月','五月','六月',
			'七月','八月','九月','十月','十一月','十二月'], // Names of months for drop-down and formatting
		monthNamesShort: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'], // For formatting
		monthStatus: '选择月份', // Status text for selecting a month
...
$.extend(Datepicker.prototype, {
	...
	_attachDatepicker: function(target, settings) {
		...
		if (!target.id){//保证唯一
			target.id = 'dp' + new Date().getTime()+Math.round(Math.random()*1000);
		}
	...
	_selectDate: function(id, dateStr) {
		...
		if(this._get(inst, 'showTime')){
			time = inst.dpDiv.find("input[name='time']").val();
		}
		if (inst.input){
			if(dateStr!=""){
	...
	_generateDatepicker: function(inst) {
		...
		if(this._get(inst, 'showTime')){
			...
			currTimeStr+='<div class="ui-datepicker-time">';
			currTimeStr+='<input type="text" size="8" value="'+currTime.getHours()+":"+currTime.getMinutes()+":"+currTime.getSeconds()+'" name="time"/>';
			currTimeStr+='</div>';
		}
		var controls = '<div class="ui-datepicker-control">' + (isRTL ? '' : clear) +
			currTimeStr+'<div class="ui-datepicker-close"><a onclick="jQuery.datepicker._hideDatepicker();"' +
